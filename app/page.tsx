...
export default function Home() {
  ...
  // Using your original working V1 environment variable
  ...
  const handleSwipe = async () => {
    if (!AWS_API_URL) {
      setSwipeStatus("Error: AWS_API_URL is missing in Vercel/Local environment.");
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-8">
        ...
        <form onSubmit={handleAuth} className="bg-slate-800 p-8 rounded-xl shadow-2xl w-full max-w-md border border-slate-700 space-y-4">
          ...
          <button type="submit" className="w-full py-3 bg-blue-600 hover:bg-blue-700 rounded font-bold">{isSignUp ? "Sign Up" : "Log In"}</button>
          <p className="text-center text-sm text-slate-400 cursor-pointer hover:text-blue-400" onClick={() => setIsSignUp(!isSignUp)}>{isSignUp ? "Already have an account? Log in" : "Need an account? Sign up"}</p>
          {message && <p className="text-center text-emerald-400 mt-2">{message}</p>}
        </form>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-200 p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
          ...
          {swipeStatus && <p className="mt-4 text-center text-emerald-400">{swipeStatus}</p>}
        </div>
              ...
              {logs.length === 0 ? (
                ...
                logs.map((log: any) => (
                  <tr key={log.id} className="border-b border-slate-700/50">
                    ...
                    <td className="p-4 text-emerald-400 font-bold">{log.aiDecision}</td>
                  </tr>
                ))
              )}
            </tbody>
